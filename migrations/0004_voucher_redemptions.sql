...
CREATE TRIGGER IF NOT EXISTS voucher_redemptions_no_update BEFORE UPDATE ON voucher_redemptions BEGIN
 SELECT RAISE(ABORT,'redemption_immutable');
END;

CREATE TRIGGER IF NOT EXISTS voucher_redemptions_no_delete BEFORE DELETE ON voucher_redemptions BEGIN
 SELECT RAISE(ABORT,'redemption_immutable');
END;

CREATE TRIGGER IF NOT EXISTS voucher_redemption_reversals_no_update BEFORE UPDATE ON voucher_redemption_reversals BEGIN
 SELECT RAISE(ABORT,'redemption_immutable');
END;

CREATE TRIGGER IF NOT EXISTS voucher_redemption_reversals_no_delete BEFORE DELETE ON voucher_redemption_reversals BEGIN
 SELECT RAISE(ABORT,'redemption_immutable');
END;
