...
CREATE TRIGGER IF NOT EXISTS voucher_change_no_update BEFORE UPDATE ON voucher_changes BEGIN
 SELECT RAISE(ABORT,'voucher_change_immutable');
END;

CREATE TRIGGER IF NOT EXISTS voucher_change_no_delete BEFORE DELETE ON voucher_changes BEGIN
 SELECT RAISE(ABORT,'voucher_change_immutable');
END;
