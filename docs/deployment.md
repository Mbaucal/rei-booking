...
**Latest checkpoint — 23 September 2026:** Cloudflare's Git trigger is working again. Build `972ae38d-e381-44ae-8dc4-89c0d0df1110` for main commit `e0a6da60f16b8a1db3b8f5748ac1e12dc1b59b38` reached the release `0.3.0` tests, then stopped before deployment. The owner supplied the complete failure log: the successful-move assertion received an expected booking conflict because the preceding concurrency test sometimes booked the same therapist at 13:00 on the shared fixture date, overlapping the moved 12:05–13:05 appointment. The concurrency test now uses its own date and verifies one persisted appointment and twelve occupied five-minute slots. Application conflict protection and deployment commands are unchanged. A new successful Cloudflare build and hosted acceptance are still required; the last confirmed hosted health response was `0.1.0`.

Release `0.3.0` includes Dashboard, Reports, Team bonus settings and Sales / Gift vouchers. Hosted acceptance remains open until the canonical health endpoint reports `0.3.0` and the owner verifies those screens. Real voucher email sending remains disabled pending sender verification and Worker secrets. Owner sign-in was already confirmed; do not repeat initial provisioning or reset the database to resolve this deployment mismatch. GitHub's `verify` job runs checks, tests and deployment dry runs only; a green GitHub check is not proof of a Cloudflare deployment.

**22 September 2026:** the user confirmed successful owner sign-in after the browser/console recovery. Initial schema, runtime origin and owner access are resolved. Release 0.2 adds Reports, Dashboard and Team bonus settings. On the first authenticated API request after deployment, the server idempotently creates `therapist_bonus_rules` and `appointment_bonus_rules`. The migration is additive and contains no UPDATE/DELETE of existing records. Its SQL matches `0002_report_bonuses.sql`, which Wrangler may later record as applied. No manual SQL, repeat owner setup or database recreation is required. Hosted verification of the new reporting screens and real-device interaction remains pending.
