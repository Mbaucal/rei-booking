...
export function voucherContent(
  ...
  const closed =
    {
      voided: "VOID — not valid",
      refunded: "REFUNDED — not valid",
      replaced: "REPLACED — old code not valid",
    }[voucher.status] || "";
  const lines = [
    closed,
...
${closed ? `<p style="font-weight:bold;color:#a62d2d;border:2px solid #a62d2d;padding:12px;margin-bottom:24px">${e(closed)}</p>` : ""}
