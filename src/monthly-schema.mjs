export const MONTHLY_SCHEMA = [
 ...
 provider_id TEXT UNIQUE, error_code TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL
  ...
  ...["report_templates", "report_snapshots"].flatMap((table) =>
    ["UPDATE", "DELETE"].map(
      (op) =>
        `CREATE TRIGGER IF NOT EXISTS ${table}_immutable_${op.toLowerCase()} BEFORE ${op} ON ${table} BEGIN
...
export async function ensureMonthlySchema(db) {
  let promise = initialized.get(db);
  if (!promise) {
    promise = db
      .batch(MONTHLY_SCHEMA.map((sql) => db.prepare(sql)))
      .catch((error) => {
